...
import {projectEnvSynchronization} from '@/apis/webUiTest/project'
...
export default {
    ...
    saveEventSynchronizer() {
      ...
      projectEnvSynchronization({
        ...
        if (this.showMessage(this, response)) {
          this.$bus.$emit(this.$busEvents.ui.uiEnvSynchronizerIsSuccess, response.data)
  ...
  mounted() {
    ...
    this.$bus.$on(this.$busEvents.ui.uiShowEnvSynchronizer, (projectId) => {
  ...
  // 组件销毁前，关闭bus监听事件
  beforeDestroy() {
    this.$bus.$off(this.$busEvents.ui.uiShowEnvSynchronizer)
  },
}
</script>
